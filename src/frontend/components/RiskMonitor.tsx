...
export function RiskMonitor({
    ...
    const hasLiveHealth = typeof healthFactor === 'number' && Number.isFinite(healthFactor);
    const hfValue = hasLiveHealth ? healthFactor : 0;

    const isSafe = hasLiveHealth && hfValue > 1.5;
    const isModerate = hasLiveHealth && hfValue > 1.1 && hfValue <= 1.5;
    const statusLabel = !hasLiveHealth ? 'Waiting' : isSafe ? 'Protected' : isModerate ? 'Attention' : 'Critical';

    const arcStroke = !hasLiveHealth ? '#6b7280' : isSafe ? '#10b981' : isModerate ? '#f59e0b' : '#ef4444';
    ...
    const hasProjectedDrop = typeof projectedDrop === 'number' && Number.isFinite(projectedDrop);

    return (
        <Card className="relative overflow-hidden border-none bg-black/40 backdrop-blur-xl h-full flex flex-col justify-between">
            <div className={`absolute top-0 right-0 p-4 opacity-20 blur-3xl w-32 h-32 rounded-full ${!hasLiveHealth ? 'bg-gray-500' : isSafe ? 'bg-emerald-500' : isModerate ? 'bg-amber-500' : 'bg-red-500'}`}></div>
            ...
            <div className="p-6 relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-start mb-2">
                    ...
                    <div className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 ${!hasLiveHealth ? 'bg-gray-500/20 text-gray-300' : isSafe ? 'bg-emerald-500/20 text-emerald-500' : isModerate ? 'bg-amber-500/20 text-amber-500' : 'bg-red-500/20 text-red-500'}`}>
                ...
                <div className="flex-1 flex items-center justify-center relative my-4">
                    ...
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-center">
                        ...
                        <div className={`text-[10px] font-bold uppercase tracking-widest ${!hasLiveHealth ? 'text-gray-300' : isSafe ? 'text-emerald-500' : isModerate ? 'text-amber-500' : 'text-red-500'}`}>
                            {hasLiveHealth ? statusLabel : 'Connect Wallet'}
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                    ...
                    <div className="bg-white/5 p-3 rounded-lg border border-white/5">
                        ...
                        <div className={`text-lg font-bold ${!hasProjectedDrop ? 'text-white' : Math.abs(projectedDrop) > 5 ? 'text-emerald-400' : 'text-red-400'}`}>
                            {hasProjectedDrop ? `${projectedDrop > 0 ? '+' : ''}${projectedDrop.toFixed(2)}%` : '--'}
                        </div>
